...
from answer_prompt import course_info,product_using,product_reading,product_transcribe,faq,reading,solver,transcribe,system_instruction_header
import logging
...
def read_csv_first_column(generator):
    """
    读取指定CSV文件并提取第一列内容
    Args:
        major_name: 专业名称
    ...
    combined_list = []
    csv_files = [
        'major_name_ai answer generator.csv',
    ]
...
def generate_seo_content(key_words_list: List[str]) -> List[Dict]:
    ...
    # 保存结果到JSON文件
    output_file = r"output/seo_content_results.json"
    try:
...
if __name__ == "__main__":
    ...
    try:
        key_words_list = read_csv_first_column("")[:2]
        
        # 将关键词列表保存为JSON文件
        json_path = "output/keywords.json"
